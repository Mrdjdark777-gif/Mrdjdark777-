...
## Живая обратная связь — четвёртый фикс за день: прямое распознавание намерения вместо тонкой подстройки score

После предыдущего фикса (регистрация термина "Duplicate" + автогенерация
спрягаемой формы глагола через pymorphy3) пользователь написал прямым
текстом: "Ответ совсем не тот, я уже не знаю какую тебе задачу дать
чтоб этот бот действительно хоть как-то начал работать". Обоснованная
жёсткая реакция — предыдущий фикс формально сработал (chunk_confident,
exact_term_bonus=1.0, тема верная), но ПОБЕДИЛА official Manual-
страница "Дублировать", которая объясняет МЕХАНИКУ операции ("копия
создаётся в том же месте... разделяет блоки данных с исходным
объектом... эту форму дублирования иногда называют «мелкой ссылкой»")
и ни разу не называет саму комбинацию Shift-D в выигравшем sub-chunk'е.
Тесты это не ловили, потому что формально это не регрессия (правильная
тема, уверенный ответ) — но пользователю такой ответ бесполезен: он
спросил КАКАЯ клавиша, получил рассказ ЧТО происходит при дублировании.

**Решение.** За один день это уже ТРЕТИЙ раунд подстройки ранжирования
вокруг hotkeys/authority-конфликта (chunk_kind_score term-scope,
SATURATION_K, регистрация термина + словоформа) — каждый раз чинил
ОДИН конкретный случай, но не саму причину: обычная authority-иерархия
(раздел 3 ТЗ, официальный Manual выше personal) в принципе не подходит
для вопросов "какая клавиша делает X", потому что для ТАКОГО вопроса
единственный источник, который в принципе может дать ПРЯМОЙ ответ —
это hotkeys-контент, вне зависимости от его authority-тира.

Вместо очередной точечной подстройки — прямое распознавание намерения
вопроса. `search/engine.py::_is_hotkey_intent()` — regex на
"хоткей"/"горячие клавиши"/"какая клавиша"/"какое сочетание клавиш"/
"комбинация клавиш"/"что нажать". Когда обнаружено — `chunk.source ==
"hotkeys"` получает `hotkey_intent_score=1.0` (все остальные chunk'и —
0.2, штраф ниже нейтрали), с весом `HOTKEY_INTENT_MODIFIER_WEIGHT=1.0`
— вдвое-впятеро больше любого другого модификатора
(authority/version/topic/chunk_kind — все 0.2-0.6). На обычные, НЕ
hotkey-намеренные вопросы модификатор не влияет вообще (нейтральное
0.5, проверено тестом
`test_non_hotkey_question_keeps_neutral_hotkey_intent_score`).

**Результат подтверждён на реальных данных, не предположен:**
- "Какой хоткей дублирует объект в Blender?" → `Shift + D — дублировать
  объект (Duplicate); копия сразу двигается за курсором...` — именно
  то, что спросили.
- "Какое сочетание клавиш инвертирует текущее выделение?" → ТОЖЕ точный
  ответ, даже ЛУЧШЕ, чем при первом фиксе этого случая: `Ctrl+I —
  Инвертировать выделение.` (из официальной страницы хоткеев раздела
  2.2) — раньше (после фикса SATURATION_K) это был неточный soft_match
  с общей Manual-страницей про Geometry Nodes.
- "Что делает комбинация Alt + N" → честный `soft_match` — комбинации
  действительно нет ни в hand-curated данных, ни в официальной странице
  keymap, это не баг, а реальный пробел в контенте.

**Changed**: `search/engine.py` (`HOTKEY_INTENT_MODIFIER_WEIGHT`,
  `_is_hotkey_intent()`, `_hotkey_intent_score()`,
  `ScoredChunk.hotkey_intent_score`), `bot/handlers/admin.py` (`/debug`
  формат)
**Added**: `tests/test_phase7_search_engine.py::HotkeyIntentTests` (4
  теста: точность regex на положительных/отрицательных примерах,
  hotkeys-чанк побеждает official-чанк на hotkey-вопросе, обычные
  вопросы не затронуты)
**Tests**: `python -m unittest discover tests` — 318 passed, 1 known
  failure (`ResponseTimeTests`), 1 skipped. Quality Score 90.1% — без
  регрессий по остальным метрикам
**Deployed**: закоммичено, запушено, на сервере `git pull --ff-only`
  чистый, один `QAService` проверен (peak RSS 162.76 МБ, ответ
  корректный), `systemctl restart blenderbot`, `journalctl` чистый
  ("Бот запущен", `getUpdates 200 OK`, без traceback), память после
  старта ~192 МБ.

Все три примера пользователя за день (Invert Selection, Alt+N,
Duplicate) теперь дают правильный, полезный ответ на реальных данных —
не только формально проходят тесты.
