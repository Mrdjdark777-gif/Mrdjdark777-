...
class BM25Index:
    ...
    K=8.0 → K=15.0 (после ТЗ v3, живая обратная связь): реальный вопрос
    "Какое сочетание клавиш инвертирует текущее выделение (Invert
    Selection) в Edit Mode?" уверенно (chunk_confident) отвечался
    контентом совершенно не в тему — коротким chunk'ом про Geometry
    Nodes-ноду "Invert Rotation", который выиграл ТОЛЬКО за счёт высокой
    плотности слова "инвертировать" в очень коротком тексте (BM25
    закономерно завышает такие chunk'и — раздел про длину документа выше)
    при полном отсутствии распознанного термина (exact_term_bonus=0).
    Перебор K∈{8,10,12,15,20} на полном Quality Suite (`calibrate_saturation.py`,
    PROJECT_PLAN.md) показал: K=10 уже переводит этот конкретный кейс из
    chunk_confident в soft_match ("Возможно, ты имел в виду...?" — честная
    переспрашивающая форма вместо уверенного неверного ответа) БЕЗ потери
    качества по остальным метрикам (retrieval accuracy не изменилась,
    source authority accuracy даже выросла 97.2%→99.1%); K=15 показал ещё
    лучший source authority (100%) при том же weighted score — выбран как
    более уверенный запас, не самое крайнее протестированное значение."""
