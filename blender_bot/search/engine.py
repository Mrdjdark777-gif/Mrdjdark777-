...
# Два совпавших слова — сигнал, ТОЛЬКО если это вся фраза целиком.
#
# Измерено, не предположено (партия 2 концептов, 2026-08-23): одной
# доли 0.66 мало, потому что у фразы из ТРЁХ токенов она требует всего
# ДВУХ совпадений, и пара общих слов вытягивала посторонний концепт:
#   "почему рендер получается шумным" -> Material
#        (фраза "рендер получается тёмным", совпали "рендер"+"получаться")
#   "как создать грань или ребро вручную (клавиша F)" -> Knife Tool
#        (фраза "как прорезать грань вручную", совпали "грань"+"вручную")
#   "скульптинг лагает на большом объекте" -> Subdivision Surface
#        (совпали "скульптинг"+"большой")
# Таких трёхтокенных фраз в реестре 45 из 208 (22%) — то есть это не
# отдельные неудачные формулировки, а системное свойство порога.
# Полный quality-suite подтвердил цену: Retrieval accuracy 74.7% -> 74.3%,
# Quality Score 91.6% -> 91.5%.
#
# Отвергнутые альтернативы (обе проверены, а не отброшены на глаз):
#  - Поднять долю до 0.70: убирает ВСЕ ложные срабатывания, но роняет
#    число сработавших концептов на quality-suite с 10 до 2, вместе с
#    верными (пострадали бы и корректные совпадения на 2 из 3).
#  - Взвесить совпадение по IDF: не работает в принципе — IDF считается
#    по корпусу Manual, а там "почему" встречается в 6 документах из
#    10720 и получает САМЫЙ ВЫСОКИЙ idf (7.41), выше чем "материал"
#    (3.20) или "чёрный" (5.10). Вопросительное слово частотно в
#    ЗАПРОСАХ и редко в ДОКУМЕНТАЦИИ, поэтому IDF документов для него —
#    неверный сигнал, и взвешивание сделало бы ровно наоборот.
#
# Это правило бьёт точно по механизму ошибки (мало АБСОЛЮТНЫХ
# совпадений), не трогая долю, поэтому сильные совпадения (полное
# совпадение фразы, 3+ слов из 4-5) продолжают работать как раньше.
# Исключение для полного совпадения оставляет рабочими короткие
...
class SearchEngine:
    ...
    def _find_term_by_concept(self, query_lemmas: set[str]) -> Term | None:
        ...
        search()). Порогов два, и они дополняют друг друга:
        CONCEPT_MIN_OVERLAP_FRACTION отсекает совпадение малой ЧАСТИ
        фразы, а CONCEPT_STRONG_OVERLAP_TOKENS — совпадение слишком
        малого ЧИСЛА слов (подробный разбор с цифрами — у самих констант
        выше).
        ...
        for term, phrase_tokens in self._concept_phrase_tokens:
            ...
            if overlap < CONCEPT_MIN_OVERLAP_TOKENS:
                continue
            # Два совпавших слова засчитываются, только если это ВСЯ
            # фраза ("remove doubles"); иначе нужно минимум три.
            if overlap < CONCEPT_STRONG_OVERLAP_TOKENS and overlap < len(phrase_tokens):
                continue
