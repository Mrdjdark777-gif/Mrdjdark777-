...
export const pushSubscriptions=sqliteTable('push_subscriptions',{id:text('id').primaryKey(),manageHash:text('manage_hash').notNull(),subscription:text('subscription').notNull(),preferences:integer('preferences').notNull().default(7),createdAt:integer('created_at').notNull(),testedAt:integer('tested_at').notNull().default(0)});
