...
import {Mic,Radio,BookOpen,Headphones,SlidersHorizontal,Home,Search,ArrowUpRight,Plus,Upload,Square,Play,Heart,Check,Volume2,FileAudio,ChevronRight,Monitor,Trash2,Eye,EyeOff,Pencil,Pin,Loader2,LogOut,Share2,Video,Music2,Camera,Send,MessageCircle,Globe,Link2,Image as ImageIcon} from 'lucide-react';
...
type Data={items:Post[];isOwner:boolean;needsSetup:boolean;signedIn:boolean;donations:DonationLink[];links:SocialLink[];live:{id:string;title:string;startedAt:number|null;cover:boolean}|null;pinned:string|null};
...
export default function Studio(){
 ...
 const goto=(v:string)=>{setView(RETIRED_VIEWS[v]??v);setFilter('all');setQuery('');setArchiveOnly(false);setPlayerExpanded(false);};
 ...
 {!author&&view==='podcasts'&&<VoiceHeader haptic={haptic}
  ...
  columns={[{kind:'podcast',label:t('nav.podcasts'),go:()=>document.querySelector('.catalog-tools')?.scrollIntoView({behavior:'smooth',block:'start'})},{kind:'video',label:t('nav.videos'),go:()=>goto('videos')},{kind:'story',label:t('nav.stories'),go:()=>goto('stories')}]}/>}
 ...
 {playing&&<PodcastPlayer key={playing.id} src={'/api/audio?id='+playing.id} title={playing.title} duration={playing.duration} cover={playing.coverKey?'/api/cover?id='+playing.id:playing.coverUrl||undefined} note={playing.description||undefined} archived={isLiveArchive(playing.audioKey)} supportUrl={heartLink?.url}
  next={(()=>{const after=nextEpisode(sectionOrder,playing.id);return after?{id:after.id,title:after.title,duration:after.duration,cover:after.coverKey?'/api/cover?id='+after.id:after.coverUrl||undefined}:null;})()}
