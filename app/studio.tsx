...
type Data={items:Post[];isOwner:boolean;needsSetup:boolean;signedIn:boolean;donations:DonationLink[];links:SocialLink[];live:{id:string;title:string;startedAt:number|null;cover:boolean}|null;pinned:string|null;calmArt?:boolean};
...
{data&&!data.needsSetup&&<nav className="bottom-nav">
 ...
 {playing&&<PodcastPlayer key={playing.id} src={'/api/audio?id='+playing.id} title={playing.title} duration={playing.duration} cover={coverSrc(playing)||undefined} note={playing.description||undefined} archived={isLiveArchive(playing.audioKey)} supportUrl={heartLink?.url}
