'use client';
import {ListenerProfile,LiveComments,CommunityModeration,useCommunityStatus} from '@/components/community/community';
...
export default function Studio(){
 ...
 // Профиль слушателя показываем только там, где сообщество вправду включено:
 // иначе кнопка вела в окно, которое ничего не умеет.
 ...
 const canDonate=!!data?.donations?.length;
 ...
 {view==='live'&&author&&!!community?.enabled&&<CommunityModeration/>}{view==='live'&&author&&<LiveArchives onEdit={id=>{const post=(data?.items??[]).find(p=>p.id===id);if(post)openEditor(post.kind as 'podcast',post);}}/>}{view==='live'&&<div className="live-layout"><section className="live-main-panel live-console">
 {author?<>
 ...
 {view==='settings'&&!author&&<section className="settings-panel"><div className="section-icon"><ShieldCheck size={22}/></div>
  ...
  <div className="legal-links"><a href="/privacy" target="_blank" rel="noopener noreferrer">{t('legal.privacy')}</a><a href="/rules" target="_blank" rel="noopener noreferrer">{t('legal.rules')}</a></div></section>}
 ...
 {view==='settings'&&<section className="settings-panel wide-panel"><div className="section-icon"><ShieldCheck size={22}/></div><h2>{t('settings.legalTitle')}</h2><p>{t('settings.legalText')}</p>
  <div className="links-grid">
   <label className="field">{t('settings.legalName')}<input type="text" maxLength={120} value={legalName} onChange={e=>setLegalName(e.target.value)}/></label>
   <label className="field">{t('settings.legalContact')}<input type="email" inputMode="email" autoComplete="off" maxLength={160} placeholder="name@example.com" value={legalContact} onChange={e=>setLegalContact(e.target.value)}/></label>
  </div>
  <button className="primary-button" onClick={()=>void run(()=>api('library',{action:'legal',name:legalName.trim(),contact:legalContact.trim()}),t('settings.legalSaved'))}><Check size={17}/>{t('settings.legalSave')}</button>
  <div className="legal-links"><a href="/privacy" target="_blank" rel="noopener noreferrer">{t('legal.privacy')}</a><a href="/rules" target="_blank" rel="noopener noreferrer">{t('legal.rules')}</a></div>
 </section>}
...
{data&&!data.needsSetup&&<LimelightNav className="bottom-nav" activeId={view} items={[
 ...
 {id:'live',label:t('nav.live'),className:'bottom-nav-item bottom-nav-live tt-pressable',onClick:()=>{haptic();goto('live');},icon:wide?<LiquidMetalButton viewMode="icon" size={38} interactive={false} icon={<Radio size={18} color="#6FE7DE"/>}/>: <Radio size={22}/>,badge:liveStatus?<span className="bottom-nav-dot" aria-hidden="true"/>:null},
 {id:'stories',label:t('nav.stories'),className:'bottom-nav-item bottom-nav-stories tt-pressable',onClick:()=>{haptic();goto('stories');},icon:wide?<LiquidMetalButton viewMode="icon" size={38} interactive={false} icon={<BookOpen size={18} color="#6FE7DE"/>}/>: <BookOpen size={22}/>},
 ...(author?[{id:'settings',label:t('header.settings'),className:'bottom-nav-item bottom-nav-settings tt-pressable',onClick:()=>{haptic();goto('settings');},icon:wide?<LiquidMetalButton viewMode="icon" size={38} interactive={false} icon={<SlidersHorizontal size={18} color="#6FE7DE"/>}/>: <SlidersHorizontal size={22}/>}]:[])
 ]}/> }
 </div>
 <input type="file" accept="audio/*,.mp3,.wav,.m4a,.webm,.ogg,.flac" ref={fileInput} onChange={pickFile} hidden/>
 {playing&&<PodcastPlayer key={playing.id+':'+playFrom} src={'/api/audio?id='+playing.id} title={playing.title} duration={playing.duration} cover={coverSrc(playing)||undefined} note={playing.description||undefined} archived={isLiveArchive(playing.audioKey)} onDonate={canDonate?()=>{haptic();setDonateOpen(true);}:undefined} from={playFrom} onShare={()=>void share('/?mode=listen&post='+playing.id,playing.title)}
 ...
 <Dialog open={profileOpen} onOpenChange={setProfileOpen}><DialogContent className="community-dialog"><DialogHeader><DialogTitle>{t('community.profile')}</DialogTitle></DialogHeader><ListenerProfile/></DialogContent></Dialog>
