...
export default function Studio(){
 ...
 function playPost(p:Post,resume=false){live.leave();setResumePlay(resume);setPlayerAutoplay(true);setPlayerExpanded(true);setPlaying(p);setTimeout(()=>{player.current?.play().catch(()=>toast.info(t('player.tapInPlayer')));},50);}
...
{data&&!data.needsSetup&&<nav className="bottom-nav">
 ...
 {playing&&<PodcastPlayer key={playing.id+(resumePlay?':resume':'')} src={'/api/audio?id='+playing.id} title={playing.title} duration={playing.duration} cover={coverSrc(playing)||undefined} note={playing.description||undefined} archived={isLiveArchive(playing.audioKey)} supportUrl={heartLink?.url} resume={resumePlay} onShare={()=>void share('/?mode=listen&post='+playing.id,playing.title)}
