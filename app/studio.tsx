...
export default function Studio(){
 ...
 const goto=(v:string)=>{setView((author?AUTHOR_RETIRED[v]:undefined)??RETIRED_VIEWS[v]??v);setFilter('all');setQuery('');setArchiveOnly(false);setPlayerExpanded(false);};
 ...
 <button className="bottom-nav-item bottom-nav-home tt-pressable" data-active={view==='home'} aria-label={t('nav.home')} onClick={()=>{haptic();goto('home');}}>{wide?<LiquidMetalButton viewMode="icon" size={46} interactive={false} icon={<img className="brand-inside-metal" src="/brand/logo.png?v=0.4.1" width="34" height="34" alt=""/>}/>:<img className="nav-brand-mark" src="/brand/logo.png?v=0.4.1" width="30" height="30" alt=""/>}<span>{t('nav.home')}</span></button>
