...
export async function POST(req: Request){try{
  ...
  if(d.action==='links'){
    ...
    if(Array.isArray(d.links)&&d.links.length&&!clean.length)throw new Error('Нужны ссылки HTTPS на известные площадки');
  ...
  if(!['podcast','story','video'].includes(kind))throw new Error('Неверный тип публикации');
  ...
  if(kind==='video'){
    ...
    if(!parseVideo(videoUrl))throw new Error('Нужна ссылка HTTPS на видео (YouTube, Rutube, VK, TikTok или файл)');
    if(videoUrl.length>2000)throw new Error('Ссылка на видео слишком длинная');
  }
...
const NOTICE={podcast:{category:2,title:'Новый подкаст True Thrills',view:'podcasts'},video:{category:2,title:'Новое видео True Thrills',view:'videos'},story:{category:4,title:'Новая история True Thrills',view:'stories'}} as const;
function notifyPost(p:{id:string;kind:string;title:string},req:Request){const n=NOTICE[p.kind as keyof typeof NOTICE]??NOTICE.story;enqueueNotice('post:'+p.id,n.category,{title:n.title,body:p.title,url:'/?mode=listen&view='+n.view+'&post='+encodeURIComponent(p.id),tag:'post:'+p.id},siteOrigin(req),86400);}
