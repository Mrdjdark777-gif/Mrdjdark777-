...
import {newDeviceToken,pushKeys,sendFcmNotice,sendNotice,siteOrigin,tokenHash,validateFcmToken,validateSubscription} from '@/lib/push';
...
export async function POST(req:Request){try{
 ...
 if(d.action==='subscribe'){
  ...
  const preferences=Number(d.preferences);if(!Number.isInteger(preferences)||preferences<0||preferences>7)throw new Error('Некорректные настройки');
  ...
  const saved=db().prepare(`INSERT INTO push_subscriptions(id,manage_hash,subscription,kind,preferences,created_at)
   SELECT ?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM push_subscriptions WHERE id=?) OR (SELECT COUNT(*) FROM push_subscriptions)<100
   ON CONFLICT(id) DO UPDATE SET preferences=excluded.preferences,subscription=excluded.subscription WHERE push_subscriptions.manage_hash=excluded.manage_hash RETURNING id`).get(id,manageHash,subJson,kind,preferences,Date.now(),id);
  if(!saved)throw new Error('Лимит пилота — 100 устройств с уведомлениями.');return result({id,token,preferences});
 }
 const id=String(d.id??''),manageHash=device(req),sub=db().prepare('SELECT subscription,kind FROM push_subscriptions WHERE id=? AND manage_hash=?').get(id,manageHash) as {subscription:string;kind:string}|undefined;
 ...
 const notice={title:'True Thrills',body:'Проверка уведомлений. Новые эфиры и публикации будут приходить сюда.',url:'/?mode=listen&view=settings',tag:'test'};
 ...
 await r.body?.cancel();if(!r.ok)throw new Error('Сервис не принял уведомление. Выключи и включи уведомления заново.');return result({accepted:true});
 }catch(e){return failure(e);}}
