...
export async function GET(req: Request){try{
  ...
  if(q.has('status')){const live=await db.select({id:broadcasts.id,title:broadcasts.title,startedAt:liveRecordings.createdAt}).from(broadcasts).leftJoin(liveRecordings,eq(liveRecordings.id,broadcasts.id)).where(and(eq(broadcasts.active,1),gt(broadcasts.heartbeat,Date.now()-90000))).orderBy(desc(broadcasts.heartbeat)).get();return result({live:live??null});}
...
export async function POST(req: Request){try{
  ...
  if(['start','stop','heartbeat','answer'].includes(String(d.action))){
    ...
    if(d.action==='heartbeat'){const b=await db.select().from(broadcasts).where(eq(broadcasts.id,String(d.id))).get();if(!b?.active)return result({error:'#err.liveGone'},409);await db.update(broadcasts).set({heartbeat:now}).where(eq(broadcasts.id,String(d.id)));if(Array.isArray(d.connected)){for(const id of d.connected.slice(0,50)){if(typeof id==='string')await db.update(peers).set({heartbeat:now}).where(and(eq(peers.id,id),eq(peers.broadcastId,b.id)));}}await db.delete(peers).where(lt(peers.heartbeat,now-60000));return result({ok:true});}
  ...
  if(d.action==='join'){
    ...
    const active=await db.select({id:peers.id}).from(peers).where(and(eq(peers.broadcastId,live.id),gt(peers.heartbeat,now-90000)));if(active.length>=50)throw new Error('#err.liveFull');
