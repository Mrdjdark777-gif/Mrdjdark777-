...
export async function POST(req: Request){try{
  ...
  if(['start','stop','heartbeat','answer'].includes(String(d.action))){
    ...
    if(d.action==='start'){
      ...
      const id=crypto.randomUUID();await db.insert(broadcasts).values({id,title:String(d.title).slice(0,160),ownerId:userId(req)!,heartbeat:now,active:1});enqueueNotice('live:'+id,1,{titleKey:'push.liveTitle',body:String(d.title).slice(0,160),url:'/?mode=listen&view=live&broadcast='+id,tag:'live:'+id},siteOrigin(req),120);return result({id});
    }
  ...
  if(d.action==='join'){
    ...
    const offer=String(d.offer??'');if(offer.length>40000)throw new Error('#err.badOffer');const parsed=JSON.parse(offer);if(parsed.type!=='offer'||typeof parsed.sdp!=='string')throw new Error('#err.badOffer');
