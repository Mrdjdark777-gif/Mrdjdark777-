...
export async function GET(req:Request){try{
 ...
 const details=await owner(req)?{freeBytes,pushQueue:getDb().$client.prepare('SELECT state,COUNT(*) AS count FROM push_outbox GROUP BY state').all(),firebaseConfigured:!!process.env.FIREBASE_SERVICE_ACCOUNT_FILE&&existsSync(process.env.FIREBASE_SERVICE_ACCOUNT_FILE),trustProxy:process.env.TRUST_PROXY==='true'}:{};
