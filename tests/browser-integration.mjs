...
const env={...process.env,DATABASE_PATH:path.join(dir,'db.sqlite'),STORAGE_DIR:path.join(dir,'storage'),SESSION_SECRET:'ui-test-secret-not-production',ADMIN_PASSWORD:'ui-test-password',FIREBASE_SERVICE_ACCOUNT_FILE:''};
...
try{
 ...
 await page.locator('.podcast-toggle').click();await page.screenshot({path:'outputs/ui/player-mobile.png',fullPage:true});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
 await page.goto(base+'/?mode=listen&view=stories&post='+story.id);await page.locator('.reader-scroll').waitFor();await page.locator('.reader-options select').selectOption('22');await page.locator('.reader-scroll').evaluate(el=>el.scrollTop=500);await page.waitForTimeout(150);await page.screenshot({path:'outputs/ui/reader-mobile.png',fullPage:true});
...
}finally{server.kill('SIGTERM');await new Promise(resolve=>{if(server.exitCode!==null)resolve();else server.once('exit',resolve);});await rm(dir,{recursive:true,force:true});}
