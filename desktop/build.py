...
with tempfile.TemporaryDirectory(prefix='true-thrills-build-') as temp:
 ...
 setup_rc=work/'setup.rc';base=(ROOT/'client.rc').read_text().replace('"app.ico"','"'+str(ROOT/'app.ico')+'"').replace('"app.manifest"','"'+str(ROOT/'app.manifest')+'"').replace('True Thrills Desktop','True Thrills Setup').replace('TrueThrills.exe','TrueThrills-Setup-0.4.1.exe')
 ...
 installer=out/'TrueThrills-Setup-0.4.1.exe';run(*common,ROOT/'setup.cpp',setup_res,'-o',installer,*libs)
