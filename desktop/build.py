...
with tempfile.TemporaryDirectory(prefix='true-thrills-build-') as temp:
 ...
 # Имя установщика берётся из ресурса версии, а не из строки в скрипте:
 # иначе свежая сборка скачивается под старым номером.
 version=re.search(r'VALUE "ProductVersion", "([0-9.]+)"',(ROOT/'client.rc').read_text()).group(1)
 setup_name='TrueThrills-Setup-'+version+'.exe'
