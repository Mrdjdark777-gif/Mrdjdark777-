> Историческая памятка: для прежнего WebRTC-пути. Эфир с 0.9 идёт по HLS
> поверх HTTPS и TURN не требует. В коде приложения WebRTC больше нет вовсе:
> обработчик `/api/ice` и разбор SDP удалены как мёртвые. Поднимать coturn по
> этой памятке имеет смысл только вместе с возвратом WebRTC в приложение.
