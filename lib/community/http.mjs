...
function config(){const privacy=process.env.COMMUNITY_PRIVACY_URL||'',contact=process.env.COMMUNITY_CONTACT_EMAIL||'';let valid=false;try{valid=new URL(privacy).protocol==='https:'&&/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(contact)&&!privacy.includes('CHANGE_ME')&&!contact.includes('CHANGE_ME');}catch{}return {enabled:process.env.COMMUNITY_ENABLED==='true'&&valid,privacy,contact,rules:RULES};}
function token(req){return req.headers.get('cookie')?.split(';').map(x=>x.trim()).find(x=>x.startsWith(COOKIE+'='))?.slice(COOKIE.length+1)||'';}
function origin(req){const configured=new URL(process.env.PUBLIC_SITE_URL||'');if(configured.protocol!=='https:'||req.headers.get('origin')!==configured.origin)throw Error('origin');}
