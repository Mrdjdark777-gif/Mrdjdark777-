...
export function makeCommunity({db,rateSecret,now=Date.now}){
 ...
 // Вход живёт неделю, но неделю с последнего обращения, а не с первого.
 // Иначе человек, потерявший код, через семь дней терял доступ к своему же
 // профилю — а вместе с ним и возможность выгрузить и удалить свои данные.
 function me(token){if(!/^[a-f0-9]{64}$/.test(token||''))return null;
  const hash=digest(token),m=db.prepare('SELECT m.id,m.nickname,m.banned FROM tt_members m JOIN tt_member_sessions s ON s.member_id=m.id WHERE s.hash=? AND s.expires>?').get(hash,now());
  if(!m)return null;
  // Записываем не чаще раза в час: иначе каждый опрос комментариев писал бы в базу.
  db.prepare('UPDATE tt_member_sessions SET expires=? WHERE hash=? AND expires<?').run(now()+7*86400000,hash,now()+7*86400000-3600000);
  return m;}
 ...
 function signup(ip,rules){if(rules!==RULES)throw Error('rules_required');limit('signup:'+ip,3,86400000);const id=randomUUID(),raw=randomBytes(24).toString('hex').toUpperCase(),code='TT-'+raw.match(/.{1,8}/g).join('-');
  return db.transaction(()=>{db.prepare('INSERT INTO tt_members(id,nickname,recovery_hash,created_at,rules) VALUES(?,?,?,?,?)').run(id,'TT-'+id.slice(0,8).toUpperCase(),digest(raw),now(),RULES);return {code,token:session(id),member:db.prepare('SELECT id,nickname FROM tt_members WHERE id=?').get(id)};})();
 }
