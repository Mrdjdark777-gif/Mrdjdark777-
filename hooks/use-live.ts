...
export function useLive(){
 ...
 const joinedId=useRef('');const [activeId,setActiveId]=useState('');
 ...
 function endViewer(next:ListenPhase='idle',message=''){
  joinedId.current='';setActiveId('');generation.current++;if(listenTimer.current)clearInterval(listenTimer.current);if(meterTimer.current)clearInterval(meterTimer.current);viewer.current?.close();viewer.current=null;
 ...
 async function start(title:string,s:MediaStream){
  ...
  try{
   const r=await api<{id:string}>('live',{action:'start',title});host.current=r.id;setHosting(r.id);setHostStatus(t('liveHook.started'));setHostSeconds(0);const began=Date.now();elapsedTimer.current=setInterval(()=>setHostSeconds(Math.floor((Date.now()-began)/1000)),1000);
   ...
   }catch{if(host.current!==r.id)return;errors++;setHostStatus(t('liveHook.reconnectingServer'));if(errors>=5){void stop().catch(()=>{});setHostStatus(t('liveHook.stoppedLost'));toast.error(t('liveHook.lost'));}}finally{polling=false;}};
   hostTimer.current=setInterval(()=>void poll(),3000);void poll();
  }catch(e){setHostStatus(t('liveHook.startFailed'));toast.error(errorText(e));}finally{starting.current=false;setConnecting(false);}
 }
 async function resume(){try{await context.current?.resume();await audio.current?.play();}catch{setPhase('blocked');setStatus(t('liveHook.tapToEnable'));}}
 ...
 async function listen(id:string,title=t('liveHook.title')){
  ...
  try{
   ...
   pc.ontrack=e=>{
    ...
    void el.play().catch(()=>{if(gen===generation.current){playbackBlocked=true;setConnecting(false);setPhase('blocked');setStatus(t('liveHook.soundReady'));}});
   };
   pc.onconnectionstatechange=()=>{
    if(gen!==generation.current)return;
    if(pc.connectionState==='connected'){
     disconnectedAt=0;setConnecting(false);
     if(playbackBlocked){setPhase('blocked');setStatus(t('liveHook.soundReady'));}
     else if(userPaused){setPhase('paused');setStatus(t('liveHook.paused'));}
     else if(!el.paused&&el.readyState>=2){setListening(true);setPhase('playing');setStatus(t('liveHook.listening'));}
     else{setPhase('waiting');setStatus(t('liveHook.connected'));}
    }
    if(pc.connectionState==='disconnected'){disconnectedAt=Date.now();setListening(false);setPhase('reconnecting');setStatus(t('liveHook.reconnecting'));}
    if(pc.connectionState==='failed')endViewer('error',t('liveHook.failed'));
   };
   ...
   const poll=async()=>{if(polling||gen!==generation.current)return;polling=true;try{
    ...
    if(!d.active){endViewer('ended',t('liveHook.authorEnded'));return;}if(d.answer&&!pc.currentRemoteDescription)await pc.setRemoteDescription(JSON.parse(d.answer));
    if(disconnectedAt>0&&Date.now()-disconnectedAt>40000&&pc.connectionState!=='connected')endViewer('error',t('liveHook.noSound'));
 ...
 return{hosting,connecting,listeners,status,listening,joined,activeId,phase,hostStatus,hostSeconds,volume,setVolume,level,samples,start,stop,listen,leave,resume,pause,unmute:resume};
}
