...
async function registration(){await navigator.serviceWorker.register('/sw.js',{updateViaCache:'none'});return Promise.race([navigator.serviceWorker.ready,new Promise<never>((_,reject)=>setTimeout(()=>reject(new Error(t('notif.startFailed'))),15000))]);}
