...
// При удалении нужно ещё и обнулить playlist, иначе запрос упрётся в файлы,
// которых уже нет. Поэтому на чтение — только в сухом прогоне.
const db = new Database(process.env.DATABASE_PATH || 'data/truethrills.db', {readonly: !apply});
// Та же пауза, что и в live-worker.mjs: слушатель, подключённый в конце эфира,
// дотягивает по плейлисту последние сегменты.
...
for(const entry of entries){
 ...
 const row = db.prepare('SELECT state, post_id, updated_at FROM live_recordings WHERE id=?').get(id);
 ...
 if(Date.now() - row.updated_at < CLEANUP_AFTER_MS){ console.log(`пропуск ${id}: эфир закончился только что — его ещё могут дослушивать`); kept++; continue; }
 ...
 if(apply){
  ...
  console.log(`удалено ${id} — ${human(bytes)}`);
 }
