...
const active=new Map();let quitting=false;
...
async function processRecording(row){
 ...
 const ff=spawn('ffmpeg',args,{stdio:['pipe','ignore','pipe']});let errorText='',exited=false,code=null;
 ...
 try{
  ...
  if(quitting){ff.kill('SIGTERM');await exit;return;} // Retain chunks; restart rebuilds both outputs.
