...
// Живое хранилище передаётся третьим аргументом: с ним проверка отличает файл,
// потерянный при копировании, от ссылки, под которой файла давно нет.
execFileSync(process.execPath,[fileURLToPath(new URL('./verify-backup.mjs',import.meta.url)),output,String(expectedAudio),canonicalStorage],{stdio:'inherit'});
