...
import {mkdir,cp,writeFile,realpath} from 'node:fs/promises';
...
await writeFile(path.join(output,'RESTORE.txt'),'SQLite online backup + audio storage. Stop publications/deletions while backing up for a consistent content set. Restore while the service is stopped. Secrets from .env are NOT included: save .env separately in a private backup. The DB contains private push subscription keys. Keep this archive private.\n',{mode:0o600});
