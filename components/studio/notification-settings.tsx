...
export function NotificationSettings({author=false}:{author?:boolean}){
 ...
 {n.supported?<>
  ...
  <div className="notification-options">{OPTIONS.map(([bit,key])=><label key={bit} className={n.enabled?'':'is-inactive'}><Switch checked={n.enabled&&!!(n.preferences&bit)} disabled={n.busy||!n.enabled} onCheckedChange={checked=>void n.update(checked?n.preferences|bit:n.preferences&~bit)}/>{t(key)}</label>)}</div>
  <div className="settings-actions"><button className="primary-button" disabled={n.busy} onClick={()=>void((n.enabled||n.subscribed)?n.disable():n.enable())}>{n.busy?<Loader2 className="spin" size={18}/>:<Bell size={18}/>} {(n.enabled||n.subscribed)?t('notif.disable'):t('notif.enable')}</button>{author&&n.enabled&&<button className="secondary-button" disabled={n.busy} onClick={()=>void n.test()}>{t('notif.test')}</button>}</div>
