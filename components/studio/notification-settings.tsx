...
export function NotificationSettings({author=false}:{author?:boolean}){
 ...
 {n.supported?<>
  <div className="notification-options">{OPTIONS.map(([bit,key])=><label key={bit} className={n.enabled?'':'is-inactive'}><Switch checked={!!(n.preferences&bit)} disabled={n.busy||!n.enabled} onCheckedChange={checked=>void n.update(checked?n.preferences|bit:n.preferences&~bit)}/>{t(key)}</label>)}</div>
