...
export function PodcastPlayer({src,title,duration:initialDuration=0,audioRef,onClose}:{src:string;title:string;duration?:number;audioRef:RefObject<HTMLAudioElement|null>;onClose:()=>void}){
 ...
 const session=()=>{
  ...
  navigator.mediaSession.metadata=new MediaMetadata({title,artist:'True Thrills',album:'Подкасты',artwork:[{src:location.origin+'/icon-512.png?v=0.4.1',sizes:'512x512',type:'image/png'}]});
 ...
 async function play(){try{await local.current?.play();if(alive.current)setMessage('');}catch(e){if(alive.current&&!repairing.current&&!(e instanceof DOMException&&e.name==='AbortError')){setLoading(false);setMessage('Нажми «Слушать», чтобы включить звук.');}}}
 function seek(seconds:number){const el=local.current;if(!el||!Number.isFinite(el.duration)||el.duration<=0)return;const target=Math.min(el.duration,Math.max(0,seconds));try{el.currentTime=target;setPosition(target);mediaPosition();}catch{setMessage('Не удалось перемотать. Дождись загрузки записи.');}}
 async function repair(){
  if(repairing.current)return;const el=local.current;if(!el)return;
  repairing.current=true;attempted.current=true;setLoading(true);setMessage('Подготавливаем длительность и перемотку…');
  const resumeAt=Number.isFinite(el.currentTime)?el.currentTime:0;el.pause();
  const abort=new AbortController();controller.current=abort;
  try{
   const response=await fetch(src,{signal:abort.signal});if(!response.ok)throw new Error('Не удалось загрузить подкаст');
   const size=Number(response.headers.get('content-length'));if(size>80*1024*1024)throw new Error('Запись превышает 80 МБ');
 ...
 return <section className="podcast-player" aria-label={'Плеер: '+title}>
  ...
  <button className="player-close" onClick={onClose} aria-label="Закрыть плеер"><X size={20}/></button>
  <audio ref={el=>{local.current=el;audioRef.current=el;}} preload="metadata" onLoadedMetadata={metadata} onDurationChange={metadata}
   ...
   onError={()=>{setLoading(false);setMessage('Аудио недоступно. Попробуй открыть подкаст ещё раз.');}}/>
  <div className="podcast-transport"><button onClick={()=>seek((local.current?.currentTime??0)-15)} disabled={!seekable} aria-label="Назад на 15 секунд"><RotateCcw size={19}/><span>15</span></button>
   <button className="podcast-toggle" aria-label={playing?'Пауза':'Слушать'} disabled={repairing.current} onClick={()=>{if(playing){wanted.current=false;local.current?.pause();}else{wanted.current=true;void play();}}}>{loading?<Loader2 className="spin" size={22}/>:playing?<Pause size={23}/>:<Play size={23}/>}</button>
   <button onClick={()=>seek((local.current?.currentTime??0)+15)} disabled={!seekable} aria-label="Вперёд на 15 секунд"><RotateCw size={19}/><span>15</span></button></div>
  <div className="podcast-timeline"><Slider aria-label="Перемотка подкаста" aria-valuetext={clock(position)+' из '+clock(duration)} value={[Math.min(position,duration||0)]} min={0} max={duration||1} step={0.1} disabled={!seekable} onValueChange={v=>{scrubbing.current=true;setPosition(v[0]);}} onValueCommit={v=>{scrubbing.current=false;seek(v[0]);}}/><div className="podcast-times"><span>{clock(position)}</span><span>{duration>0?clock(duration):'Определяем длительность…'}</span></div></div>
