...
type Props={src:string;title:string;duration?:number;cover?:string;note?:string;archived?:boolean;onDonate?:()=>void;onShare?:()=>void;from?:PlayFrom;next?:{id:string;title:string;cover?:string;duration:number}|null;onNext?:(id:string)=>void;audioRef:RefObject<HTMLAudioElement|null>;autoplay?:boolean;expanded?:boolean;onExpand?:(next:boolean)=>void;onClose:()=>void};

function WebPodcastPlayer({src,title,duration:initialDuration=0,cover,note,archived,onDonate,onShare,from='begin',next,onNext,audioRef,autoplay=true,expanded=true,onExpand=()=>{},onClose}:Props){
 ...
 function metadata(){const el=local.current;if(!el||el.readyState===0)return;if(Number.isFinite(el.duration)&&el.duration>0){if(!restored.current){restored.current=true;
  ...
  if(from==='resume'){const progress=readProgress().find(p=>p.id===postId);if(progress&&progress.position<el.duration-2)el.currentTime=progress.position;}}setDuration(el.duration);setSeekable(true);setLoading(false);mediaPosition();}else if(!attempted.current)void repair();}
 ...
 return <PlayerChrome expanded={expanded} onExpand={onExpand}
  view={{title,cover,note,postId,next,presentation:presentation({archived,cover}),kindLabel:archived?t('post.liveArchive'):t('post.podcast'),
  ...
  act={{toggle:()=>{if(playing){wanted.current=false;local.current?.pause();}else{wanted.current=true;void play();}},
   ...
   setRate:value=>{setRate(value);if(local.current)local.current.playbackRate=value;mediaPosition();},setSleep:value=>setSleep(Number(value)),close:onClose,openNext:onNext,share:onShare,donate:onDonate}}>
  {audio}
 </PlayerChrome>;
}
