...
/** Soft Home: real covers, rounded cards, archive in carousel, support last.
 * Live remains an explicit entry and never replaces a card under the pointer.
 * Existing navigation/player callbacks and publication visibility stay intact.
 */
const HOLD_MS=500;
type Live={id:string;title:string;cover:boolean};
const coverOf=(p:ScenePost)=>coverSrc(p);

export function HomeSceneView<T extends ScenePost>({posts,live,onOpen,onOpenLive,liveAction,archive,support,links,appLink,pinned}:{
 ...
 return <div className="immersion tt-soft-home">
  ...
  <section className="soft-catalog" aria-label={t('home.freshList')}>
   <h3>{t('home.freshList')}</h3>
   <ul className="soft-carousel" aria-label={t('home.freshList')}>
    {/* Archive is a real navigation card, never a fake playable episode. */}
    {archive&&<li className="soft-archive">{archive}</li>}
  ...
  {/* Final content block; existing global player and tab bar stay outside. */}
  <section className="soft-support">
   {support}
   {links&&<div className="soft-socials">{links}</div>}
  </section>
