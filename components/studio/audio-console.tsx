...
export function InputPicker({capture,locked=false}:{capture:Capture;locked?:boolean}){
 const {t}=useT();
 // Здесь остался один выбор — откуда идёт звук. Источник и канал входа
 // задаются в FL Studio и в Windows, а не тут: это были настройки, которые
 // владелец не меняет, и они только занимали место на пульте. Разрешение на
 // микрофон запрашивается при подключении, отдельная кнопка для этого не нужна.
