...
export function StoryReader({id,body}:{id:string;body:string}){
 ...
 return <><label className="reader-options">{t('reader.size')} <select value={size} onChange={e=>{const next=Number(e.target.value);setSize(next);save(next);}}>{[16,19,22,26].map(value=><option key={value} value={value}>{value}</option>)}</select></label><div className="reader-progress-track"><ScrollProgress containerRef={root} label="Прогресс чтения рассказа"/></div><div ref={root} className="reading-body reader-scroll" style={{fontSize:size}} onScroll={()=>save(size)}>{body}</div></>;
}
