...
export function ListenerHighlights<T extends Item>({items,onOpen}:{items:T[];onOpen:(post:T)=>void}){
 const {t}=useT(),[progress,setProgress]=useState<Progress[]>([]);
 useEffect(()=>{const update=()=>setProgress(readProgress());const timer=setTimeout(update,0);window.addEventListener('tt-progress',update);return()=>{clearTimeout(timer);window.removeEventListener('tt-progress',update);};},[]);
 const visible=items.filter(p=>p.published===1),saved=progress.find(p=>p.position>0&&visible.some(item=>item.id===p.id&&item.kind==='podcast')),resume=visible.find(p=>p.id===saved?.id),latest=[...visible].sort((a,b)=>b.createdAt-a.createdAt)[0];
